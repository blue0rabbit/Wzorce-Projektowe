...
public abstract class CompositeShape extends Shape {
	...
	/**
	 * @param type
	 * 			Type of shapes to return from shape list
	 * @return
	 * 			Shapes list of specified type
	 */
	public List<Shape> getShapesByType(ShapeType type) {
		List<Shape> listOfShapes = new ArrayList<Shape>();
		for (Shape shape : shapes)
			if(shape.getType()==type) listOfShapes.add(shape);
		return listOfShapes;
	}
	...
	public List<Shape> getLeafShapes() {
		...
		for(Shape shape : shapes){
			if(shape instanceof LeafShape) shapeList.add(shape);
		}
		return shapeList;
	}
